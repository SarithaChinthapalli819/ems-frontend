...
export default function Leaves() {
    ...
    const applyLeave = async (userId,dateFrom,dateTo) =>{
      try{
        const response = await axios.post('https://ems-frontend-air2.vercel.app/api/leaves/applyleave',{userId,dateFrom,dateTo})
    ...
    const setAppliedLeaves = async  () =>{
        const response = await axios.get('https://ems-frontend-air2.vercel.app/api/leaves/myleaves',{
            headers:{
    ...
    const setMyApprovalLeaves = async  () =>{
      const response = await axios.get(`https://ems-frontend-air2.vercel.app/api/leaves/myApprovals/${role}`,{
          headers:{
              authorization:`Bearer ${localStorage.getItem("token")}`
          }
      })
    ...
    const leaveStatusCahange=async (id,cancel,approve,reject,email,name,leavefrom,leaveto)=>{
      const response = await axios.post(`https://ems-frontend-air2.vercel.app/api/leaves/cancelOrapproveOrreject/${id}`,{cancel,approve,reject,email,name,leavefrom,leaveto})
      if(response.data.success){ 
