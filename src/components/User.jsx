...
export default function User() {
  ...
  const editUser = async (name,email,role)=>{
    const response =await axios.put(`http://localhost:5000/api/auth/edituser/${currentUser._id}`,{name,email,role})
    if(response.data.success) {
  ...
  const setusers = async ()=>{
    const response =await axios.get(`http://localhost:5000/api/user/${isActive}`,{
      headers:{
  ...
  const setIsDelete =async (value)=>{
    if(value){
      const response = await axios.delete(`http://localhost:5000/api/auth/deleteuser/${deleteId}`)
      if (response.data.success) {
        setusers()
  ...
  const handleFileUpload =async (e) => {
    ...
    if (file) {
      ...
      reader.onload = async (event) => {
        ...
        setData(jsonData);
        try{
        const response =await axios.post('http://localhost:5000/api/auth/addMulitUsers',{jsonData})
        if(response.data.success){
