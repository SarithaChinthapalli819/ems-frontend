...
export default function User() {
  ...
  const addUser = async (name,email,password,role)=>{
    const response =await axios.post('https://ems-frontend-air2.vercel.app/api/auth/register',{name,email,password,role})
  ...
  const editUser = async (name,email,role)=>{
    const response =await axios.put(`https://ems-frontend-air2.vercel.app/api/auth/edituser/${currentUser._id}`,{name,email,role})
    if(response.data.success) {
  ...
  const setusers = async ()=>{
    const response =await axios.get(`https://ems-frontend-air2.vercel.app/api/user/${isActive}`,{
      headers:{
  ...
  const setIsDelete =async (value)=>{
    if(value){
      const response = await axios.delete(`https://ems-frontend-air2.vercel.app/api/auth/deleteuser/${deleteId}`)
      if (response.data.success) {
        setusers()
  ...
  const handleFileUpload =async (e) => {
    ...
    if (file) {
      ...
      reader.onload = async (event) => {
        ...
        setData(jsonData);
        try{
        const response =await axios.post('https://ems-frontend-air2.vercel.app/api/auth/addMulitUsers',{jsonData})
        if(response.data.success){
