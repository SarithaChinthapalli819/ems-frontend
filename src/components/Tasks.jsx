...
export default function Tasks({row,openTaskPage}) {
  ...
  const addProjectMember = async ()=>{
    const response =await axios.get(`http://localhost:5000/api/board/getMember/${row._id}`)
  ...
  const addTaks = async(taskName,userId,selectedDateFrom,projectId,priority)=>{
    ...
    const response = await axios.post('http://localhost:5000/api/tasks/AddTasks',{taskName,userId,selectedDateFrom,projectId,priority},{
        headers:{
  ...
  const editTasks = async(taskName,userId,selectedDateFrom,projectId,priority)=>{
    ...
    const response = await axios.put(`http://localhost:5000/api/tasks/updateTasks/${currentTask._id}`,{taskName,userId,selectedDateFrom,projectId,priority})
    if(response.data.success)
    {
  ...
  const deleteTasks = async(id)=>{
    ...
    const response = await axios.delete(`http://localhost:5000/api/tasks/deleteTasks/${id}`)
    if(response.data.success)
    {
  ...
  const tasksValue = async()=>{
    const response = await axios.get(`http://localhost:5000/api/tasks/getTasks/${row._id}`,{
      headers:{
          authorization:`Bearer ${localStorage.getItem("token")}`
      }
  })
  ...
  const statusChange =async (title,id)=>{
    const response = await axios.put(`http://localhost:5000/api/tasks/updateTaskStatus/${id}`,{title},{
      headers:{
          authorization:`Bearer ${localStorage.getItem("token")}`
      }
  })
