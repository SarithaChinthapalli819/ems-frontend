...
export default function Tasks({row,openTaskPage}) {
  ...
  const addProjectMember = async ()=>{
    const response =await axios.get(`https://ems-frontend-air2.vercel.app/api/board/getMember/${row._id}`)
  ...
  const addTaks = async(taskName,userId,selectedDateFrom,projectId,priority)=>{
    ...
    const response = await axios.post('https://ems-frontend-air2.vercel.app/api/tasks/AddTasks',{taskName,userId,selectedDateFrom,projectId,priority},{
        headers:{
  ...
  const editTasks = async(taskName,userId,selectedDateFrom,projectId,priority)=>{
    ...
    const response = await axios.put(`https://ems-frontend-air2.vercel.app/api/tasks/updateTasks/${currentTask._id}`,{taskName,userId,selectedDateFrom,projectId,priority})
    if(response.data.success)
    {
  ...
  const deleteTasks = async(id)=>{
    ...
    const response = await axios.delete(`https://ems-frontend-air2.vercel.app/api/tasks/deleteTasks/${id}`)
    if(response.data.success)
    {
  ...
  const tasksValue = async()=>{
    const response = await axios.get(`https://ems-frontend-air2.vercel.app/api/tasks/getTasks/${row._id}`,{
      headers:{
          authorization:`Bearer ${localStorage.getItem("token")}`
      }
  })
  ...
  const statusChange =async (title,id)=>{
    const response = await axios.put(`https://ems-frontend-air2.vercel.app/api/tasks/updateTaskStatus/${id}`,{title},{
      headers:{
          authorization:`Bearer ${localStorage.getItem("token")}`
      }
  })
