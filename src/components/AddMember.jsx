...
export default function AddMember({closeMemberPopup,teamId}) {
    ...
    const addTeamMember = async ()=>{
        const response = await axios.get(`https://ems-server.onrender.com/api/teams/user/${teamId}`)
        setTeammeber(response.data.team.user)
    }
    const addUser = async (userId) =>{
        try{
        const response = await axios.post(`https://ems-server.onrender.com/api/teams/adduser/${teamId}`,{userId})
    ...
    const removeUser = async (userId) =>{
      const response = await axios.put(`https://ems-server.onrender.com/api/teams/removeuser/${teamId}`,{userId})
      if(response.data.success) 
        {
