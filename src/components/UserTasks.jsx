...
function UserTasks() {
  ...
  const statusChange =async (title,id)=>{
    ...
    if(response.data.success){
      const response = await axios.get(`http://localhost:5000/api/tasks/getMyTasks/${role}`,{
        headers:{
            authorization:`Bearer ${localStorage.getItem("token")}`
        }
    })
  ...
  const onDragEnd =async (result) => {
    ...
    if (startColumn === endColumn) {
      ...
      const response = await axios.put(`http://localhost:5000/api/tasks/updateTaskStatus/${draggableId}`,{title},{
        headers:{
            authorization:`Bearer ${localStorage.getItem("token")}`
        }
    })
