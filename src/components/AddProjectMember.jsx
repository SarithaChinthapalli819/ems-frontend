...
export default function AddProjectMember({openAddMemberPopup,id}) {
    ...
    const addProjectMember = async ()=>{
       ...
       const userresponse = await axios.get('https://ems-server.onrender.com/api/user/1',{
        headers:{
    ...
    const addUserToTeam = async (value)=>{
        const member=value
        try{
       const response=await axios.post(`https://ems-server.onrender.com/api/board/addMember/${id}`,{member})
    ...
    const removeUser = async (userId) =>{
        const member=userId
        try{
       const response=await axios.put(`https://ems-server.onrender.com/api/board/removeMember/${id}`,{member})
       if(response.data.success){
