...
export default function AddProjectMember({openAddMemberPopup,id}) {
    ...
    const addProjectMember = async ()=>{
       const response =await axios.get(`http://localhost:5000/api/board/getMember/${id}`)
    ...
    const addUserToTeam = async (value)=>{
        const member=value
        try{
       const response=await axios.post(`http://localhost:5000/api/board/addMember/${id}`,{member})
    ...
    const removeUser = async (userId) =>{
        const member=userId
        try{
       const response=await axios.put(`http://localhost:5000/api/board/removeMember/${id}`,{member})
       if(response.data.success){
