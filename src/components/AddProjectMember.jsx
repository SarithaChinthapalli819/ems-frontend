...
export default function AddProjectMember({openAddMemberPopup,id}) {
    ...
    const addProjectMember = async ()=>{
       const response =await axios.get(`https://ems-server-pi.vercel.app/api/board/getMember/${id}`)
       setTeammeber(response.data.members[0].User)
       const members=response.data.members[0].User
       const userresponse = await axios.get('https://ems-server-pi.vercel.app/api/user/1',{
        headers:{
    ...
    const addUserToTeam = async (value)=>{
        const member=value
        try{
       const response=await axios.post(`https://ems-server-pi.vercel.app/api/board/addMember/${id}`,{member})
    ...
    const removeUser = async (userId) =>{
        const member=userId
        try{
       const response=await axios.put(`https://ems-server-pi.vercel.app/api/board/removeMember/${id}`,{member})
       if(response.data.success){
