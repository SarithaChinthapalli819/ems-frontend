...
export default function App() {
  ...
  const getUsers=async ()=>{
    ...
    if(response.data.users.length == 0){
      const response=await axios.post('https://ems-server-pi.vercel.app/api/auth/register',{name:'Admin',email:'Admin@gmail.com',password:'Test123!',role:'Admin'})
  ...
  const verify = async ()=>{
    ...
    const response=await axios.get('https://ems-server-pi.vercel.app/api/auth/verify',{
      headers:{
        authorization:`Bearer ${localStorage.getItem("token")}`
      }
    })
